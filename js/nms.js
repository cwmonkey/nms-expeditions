...
function htmlToTxt(text) {
  text = text.replace(/<a ([^>]+)>(.+?)<\/a>/g, function(match, p1, p2) {
    let href = p1.replace(/.*href="([^"]+)".*/, "$1");
    if (href.substr(0, 1) === '#') href = `https://cwmonkey.github.io/nms-expeditions/${href}`;
    if (href.substr(0, 1) === '/') href = `https://cwmonkey.github.io/nms-expeditions${href}`;
    return `${p2} [ ${href} ]`;
  });
...
$('#download_all_nexus').on('click', async () => {
  // Load JSZip dynamically if not already present
  if (typeof JSZip === "undefined") {
    await new Promise((resolve, reject) => {
      const script = document.createElement("script");
  ...
  // Load turndown dynamically if not already present
  ...
  instructions = `You can use the SEASON_DATA_CACHE_S22.JSON files in this zip file to replay past No Man's Sky expeditions on PC/Mac/SteamDeck (and any other systems which allow you access to the filesystem).

After unzipping the files, choose the directory with the name corresponding to the expedition you wish to play and copy the SEASON_DATA_CACHE_S22.JSON to your NMS cache directory:

${instructions}

These files have been generated by my No Man's Sky Expedition Generator on https://cwmonkey.github.io/nms-expeditions/ - using the site you can also customize your expedition experience.
...
${notes}`;
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "ReLive-NoMansSky-Expeditions-Offline-v" + (new Date()).toISOString().substring(0,10).replace(/-/g, '.') + ".zip";
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
